...
echo "Starting data-only restore from $BACKUP_DIR"

# Restore CouchDB data
./restore-couchdb.sh "$BACKUP_DIR"

# Restore configuration files
if [ -f "$BACKUP_DIR/config.tar.gz" ]; then
  echo "Restoring configuration files"
  mkdir -p /tmp/config-restore
  tar -xzf "$BACKUP_DIR/config.tar.gz" -C /tmp/config-restore
  
  # Copy configuration without overwriting container settings
  cp -n /tmp/config-restore/config/* ../docker/config/
  
  # Clean up
  rm -rf /tmp/config-restore
fi

echo "Data restore completed. You may need to restart services:"
